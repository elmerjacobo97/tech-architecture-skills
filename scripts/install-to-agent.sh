...
case "$AGENT" in
  ...
  codex)
    ...
    tmp_block="$(mktemp)"
    {
      ...
      echo
      echo "Installed from https://github.com/elmerjacobo97/tech-architecture-skills. Each entry is a workflow you can invoke by reading the linked file and following its steps."
      echo
      skill_dirs | while IFS= read -r src; do
        name="$(basename "$src")"
        desc="$(fm "$src/SKILL.md" description)"
